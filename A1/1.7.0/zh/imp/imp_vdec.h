...
/**
 * 定义码流发送方式枚举（目前仅支持VIDEO_MODE_FRAME）
 */
typedef enum{
...
/**
 * 定义解码模式枚举（目前仅支持VIDEO_DEC_MODE_IP）
 */
typedef enum{
...
/**
 * 定义解码输出顺序枚举（目前仅支持VIDEO_OUTPUT_ORDER_DEC）
 */
typedef enum{
...
typedef struct{
	...
	PIXEL_FORMAT_E	enOutputFormat; /* 输出格式，目前仅支持NV12、ARGB1555 */
 ...
 */
typedef struct{
	IMP_U32		u32FrameCnt; /* 缓存帧个数 */
	IMP_U64		u64CacheTime; /* 缓存总时间（单位:us） */
} VDEC_CACHE_STREAM_INFO_S;
 ...
 * @attention 其他:视频输出模块（VO）根据用户设置的PTS播放
 *
 */
...
/**
 * @fn IMP_S32 IMP_VDEC_GetFrameEXT(IMP_S32 s32DevId, IMP_S32 s32ChnId, FRAME_INFO_S **pstFrameInfo, IMP_U32 *YVirAddr,IMP_U32 *CVirAddr);
 ...
 * @param[in] s32ChnId Channel号,取值范围:[0, @ref NR_MAX_VDEC_CHN - 1]
 * @param[in] pstFrameInfo 帧信息指针
 * @param[out] YVirAddr 获取到的Y虚拟地址
 * @param[out] CVirAddr 获取到的UV虚拟地址
 *
 * @retval 0 成功
 * @retval 非0 失败
 *
 ...
 * @attention 若取不到帧，会返回失败。
 */
IMP_S32 IMP_VDEC_GetFrameEXT(IMP_S32 s32DevId, IMP_S32 s32ChnId, FRAME_INFO_S **pstFrameInfo, IMP_U32 *YVirAddr,IMP_U32 *CVirAddr);
 ...
 * @remarks 回放模式下，解码模块以阻塞方式获取输出Buffer，若后级模块将输出Buffer全部占用，解码也会停止，直到获取到输出Buffer。
 *
 ...
 * @attention 创建视频缓存池的大小请从IMP_VDEC_GetBufferSize获取。
 */
...
/**
 * @fn IMP_S32 IMP_VDEC_AttachChnVbPool(IMP_S32 s32DevId, IMP_S32 s32ChnId, IMP_U32 u32PoolId)
 *
 * 将解码通道绑定到指定视频缓存池
 *
 * @param[in] s32DevId Device号,取值范围:[0, @ref NR_MAX_VDEC_DEV - 1]
 ...
 * @remarks 无。
 *
 * @attention 如果需要将解码通道绑定到指定缓存池，必须在解码Device创建之前调用该接口。
 * @attention 创建视频缓存池的大小请从IMP_VDEC_GetChnBufferSize获取。
 */
IMP_S32 IMP_VDEC_AttachChnVbPool(IMP_S32 s32DevId, IMP_S32 s32ChnId, IMP_U32 u32PoolId);

/**
 * @fn IMP_S32 IMP_VDEC_DetachChnVbPool(IMP_S32 s32DevId, IMP_S32 s32ChnId)
 *
 * 将解码通道绑定到指定视频缓存池
 *
 * @param[in] s32DevId Device号,取值范围:[0, @ref NR_MAX_VDEC_DEV - 1]
 ...
 * @attention 如果需要将解码设备从缓存池解绑，必须在解码Device销毁之后调用该接口。
 */
IMP_S32 IMP_VDEC_DetachChnVbPool(IMP_S32 s32DevId, IMP_S32 s32ChnId);
...
/**
 * @fn IMP_S32 IMP_VDEC_GetChnBufferSize(VDEC_BUFFER_PARAM_S *pstBufferParam)
 *
 * 获取解码通道所需内存大小
 *
 * @param[in] pstBufferParam 解码buffer参数
 ...
 * @attention 无。
 */
IMP_S32 IMP_VDEC_GetChnBufferSize(VDEC_BUFFER_PARAM_S *pstBufferParam);
 ...
 *
 * 由于网络波动会产生的码流堆积，设置解码器快速解码的速率
 *
 * @param[in] s32DevId Device号,取值范围:[0, @ref NR_MAX_VDEC_DEV - 1]
 * @param[in] u32Rate 快速解码的速率,取值范围:[0, 10]
 *
 * @retval =0 成功
 * @retval <0 失败
 *
 * @remarks 无。
 *
 * @attention 快速解码的速率只在预览模式并且启用时间戳时才生效。
 * @attention u32Rate默认为0，即不开启快速解码；1表示解码时间间隔减少10%，以此类推，直到消除网络延时导致的积压帧。
 */
...
/**
 * @fn IMP_S32 IMP_VDEC_GetCacheStreamInfo(IMP_S32 s32DevId, VDEC_CACHE_STREAM_INFO_S *pstCacheStrInfo)
 *
 * 获取解码缓存码流信息
 *
 * @param[in] s32DevId Device号,取值范围:[0, @ref NR_MAX_VDEC_DEV - 1]
 * @param[out] pstCacheStrInfo 缓存码流信息
 *
 * @retval =0 成功
 * @retval <0 失败
 *
 * @remarks 无。
 *
 * @attention 必须开启PTS功能后，调用该接口才能生效。
 * @attention 必须确保PTS信息准确，否则会导致缓存时间异常。
 */
IMP_S32 IMP_VDEC_GetCacheStreamInfo(IMP_S32 s32DevId, VDEC_CACHE_STREAM_INFO_S *pstCacheStrInfo);
