 ...
 */
typedef enum{
	IMP_SCALER_METHOD_NEAR = 0, /**< 临近插值算法 */
	IMP_SCALER_METHOD_BICUBIC,	/**< 双三次插值算法 */
} IMP_SCALER_METHOD_E;
 ...
 * 设置IMP系统缩放算法类型.
 *
 * @param[in] enScalerMethod 系统缩放算法枚举,系统默认值为IMP_SCALER_METHOD_BICUBIC.
 *
 * @retval IMP_SUCCESS 成功
 * @retval 非IMP_SUCCESS 失败，具体原因参见错误码说明.
 *
 ...
 * 获取IMP系统缩放算法类型.
 *
 * @param[in] penScalerMethod 系统缩放算法枚举指针.
 *
 * @retval IMP_SUCCESS 成功
 * @retval 非IMP_SUCCESS 失败，具体原因参见错误码说明.
 *
...
/**
 * @fn IMP_S32 IMP_System_CreatPool_From_Tail(IMP_U32 *pu32PoolId, IMP_U32 u32BlkSize, IMP_U32 u32BlkCnt, const char *pcPoolName);
 *
 * 在Rmem上创建mempool,并从尾部开始创建
 *
 * @param[in] pu32PoolId 	申请的缓存池ID存放地址.
 * @param[in] u32BlkSize	申请缓存池中每个缓存块大小,单位 bytes.
 * @param[in] u32BlkSize	申请缓存池中缓存块的个数.
...
/**
 * @fn IMP_S32 IMP_System_CreatPool_Align_From_Tail(IMP_U32 *pu32PoolId, IMP_U32 u32BlkSize, IMP_U32 u32BlkCnt, const char *pcPoolName,IMP_S32 s32AlignWidth);
 *
 * 在Rmem上创建mempool,并从尾部开始创建
 *
 * @param[in] pu32PoolId 	申请的缓存池ID存放地址.
 ...
 *
 * 从对应u32PoolId 或 pcPoolName中获取block对应地址空间,此接口从rmem后面剩余空间申请
 *
 * @param[in] u32PoolId 	需要指定poolid，优先进行匹配，如果匹配成功，则忽略pciPoolName.
 * @param[in] pcName		pool的名称, 在u32PoolId 无效时，进行匹配，如果匹配成功，则从对应的pool中分配block.
 * @param[in] Size		 	申请block大小.
 ...
 * @param[in] pBlockVaddr	缓存块的虚拟地址
 * @param[in] u32Size		缓存块的大小
 * @param[in] u32Dir		刷cache的方向，DMA_BIDIRECTIONAL = 0 DMA_TO_DEVICE = 1 DMA_FROM_DEVICE = 2 DMA_NONE = 3
 *
 *
 * @retval IMP_SUCCESS 成功
 * @retval 非IMP_SUCCESS 失败，具体原因参见错误码说明.
