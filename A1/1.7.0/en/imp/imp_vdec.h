...
typedef struct{
	...
	IMP_U64		u64PTS; /* stream timestamp(unit:us) */
...
/**
 * The structure of cache stream infomation
 */
typedef struct{
	IMP_U32		u32FrameCnt; /* number of cached frames */
	IMP_U64		u64CacheTime; /* total cache time(unit:us) */
 ...
 * @param[in] s32ChnId Channel id,range:[0, @ref NR_MAX_VDEC_CHN - 1]
 * @param[in] pstFrameInfo Pointer of frame
 * @param[out] YVirAddr Y virtual address
 * @param[out] CVirAddr UV virtual address
 *
 * @retval 0 Success
 * @retval Non 0 Failure
 *
 ...
 *
 * Attach decoding channel to a specific video buffer pool
 *
 * @param[in] s32DevId Device id,range:[0, @ref NR_MAX_VDEC_DEV - 1]
 * @param[in] s32ChnId Channel id,range:[0, @ref NR_MAX_VDEC_CHN - 1]
 * @param[in] u32PoolId Video buffer pool id
 ...
 * @remarks Null
 *
 * @attention The interface must be called before the decoding channel is created.
 * @attention Using IMP_VDEC_GetChnBufferSize to get the required memory size for decoding channel.
 */
 ...
 *
 * Dettach decoding channel to a specific video buffer pool
 *
 * @param[in] s32DevId Device id,range:[0, @ref NR_MAX_VDEC_DEV - 1]
 * @param[in] s32ChnId Channel id,range:[0, @ref NR_MAX_VDEC_CHN - 1]
 * @param[in] u32PoolId Video buffer pool id
 ...
 *
 * Get the required memory size for decoding channel.
 *
 * @param[in] pstBufferParam Pointer of decoding buffer parameter
 *
 * @retval >0 Success
 ...
 *
 * Get decoding cache stream infomation
 *
 * @param[in] s32DevId Device id,range:[0, @ref NR_MAX_VDEC_DEV - 1]
 * @param[out] pstCacheStrInfo cache stream infomation
 *
 * @retval =0 Success
 * @retval <0 Failure
 *
 * @remarks Null.
 *
 * @attention This interface can take effect only after the PTS function is enabled.
 * @attention It is necessary to ensure the accuracy of PTS information, otherwise it may cause abnormal cache time.
 */
