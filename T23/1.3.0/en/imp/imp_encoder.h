 ...
 *
 * direct_mode=3 is Treble Sensor IVDC. In this mode:
 * Framesource0:Main sensor Mian channel; Framesource3:Second sensor Main channel; Framesource6:Third sensor Main channel.
 * Encoder_Grouop0 band to Framesource0; Encoder_Grouop1 bind to Framesource3; Encoder_Grouop2 bind to Framesource6.
 * Enc_chn0(h264) register to Encoder_Group0, use for Main sensor Main channel's bitstream.
 * Enc_chn1(h264) register to Encoder_Group1, use for Second sensor Main channel's bitstream.
 * Enc_chn4(h264) register to Encoder_Group2, use for Third sensor Main channel's bitstream.
 ...
 *
 * Third Sensor main channel:       |--enc_chn4 for third sensor main channel's H264
 * Framesource6 -- Encoder_Group2->
 *                                  |--enc_chn5 for third sensor main channel's JPEG (If not captured, please ignore)
 *
 * Framesource1,Framesource2,Framesource4,Framesource5,Framesource7,Framesource8 are No Double Sensor IVDC, Encoder channel and Group don't have special restrictions.
 */
...
typedef struct {
	bool    enable;         /**< true:enable GDR,false:disable GDR*/
	int     gdrCycle;       /**< GDR interval:[3,65535], defualt = 120*/
	int     gdrFrames;      /**< GDR num:[2,10], default = 6*/
} IMPEncoderGDRCfg;
 ...
 * @remark if you want to use IMP_ISP_SetFrameDrop() to set isp framerate, you need to first call this current function in order to set encoder framerate
...
/**
 * @brief int IMP_Encoder_Setframelossthd(int encChn, int framelossthd);
 *
 * Set the maximum size for a single frame
 *
 * @param[in] encChn encode channel num, range:[0, @ref NR_MAX_ENC_CHN - 1]
 * @param[in] framelossthd The maximum single frame size(byte), frames exceeding this size will be discarded, and setting the frameloss value too small may result in frame loss in the stream
 *
 * @retval 0 success
 * @retval not 0 failure
 *
...
/**
 * @brief int IMP_Encoder_Getframelossthd(int encChn, uint32_t *framelossthd)
 *
 * Get the maximum size for a single frame
 *
 * @param[in] encChn encode channel num, range:[0, @ref NR_MAX_ENC_CHN - 1]
 * @param[out] framelossthd The maximum single frame size(byte), frames exceeding this size will be discarded, and setting the frameloss value too small may result in frame loss in the stream
 *
 * @retval 0 success
 * @retval not 0 failure
 *
 ...
 *
 * Dynamically set MaxPictureSize of I frame and P frame.T23 does not support separate control of P-frame size. The input parameter MaxPictureSize_P will be internally forced to align with MaxPictureSize_I.
 *
 * @param[in] encChn encode channel num, range:[0, @ref NR_MAX_ENC_CHN - 1]
 * @param[in] MaxPictureSize_I MaxFrameISize，unit："Kbit"
 * @param[in] MaxPictureSize_P MAxFramePSize，unit："Kbit
 *
 * @retval 0 success
 * @retval ~0 failed
 *
...
/**
 * @fn int IMP_Encoder_GetChnMaxPictureSize(int encChn, uint32_t *MaxPictureSize_I, uint32_t *MaxPictureSize_P)
 *
 * Dynamically Get MaxPictureSize of I frame and P frame.T23 does not support separate control of P-frame size. The input parameter MaxPictureSize_P will be internally forced to align with MaxPictureSize_I.
 *
 * @param[in] encChn encode channel num, range:[0, @ref NR_MAX_ENC_CHN - 1]
 * @param[out] MaxPictureSize_I MaxFrameISize，unit："Kbit"
 * @param[out] MaxPictureSize_P MAxFramePSize，unit："Kbit
 *
 * @retval 0 success
 * @retval ~0 failed
 *
 ...
 * @attention This API is only applicable to H264 encoding channels.
 */
int IMP_Encoder_GetChnMaxPictureSize(int encChn, uint32_t *MaxPictureSize_I, uint32_t *MaxPictureSize_P);

/**
 * @brief int IMP_Encoder_SetMultiSectionMode(int mode, int size，int cnt)
 * Enable Multiple Section function
 *
 * @param[in] mode Multiple Section Mode
 * 0、close Multiple Section function
 * 1、open Multiple Section function,output as one frame stream
 * 2、open Multiple Section function,output as bitstream length size*cnt
 *
 * @param[in] size each section size (KByte)
 * @param[in] cnt vpu bitstream buff count.Currently,only supports 1.
 *
 * @retval 0 success
 * @retval not 0 failure
 *
 * @remark call this interface before calling IMP_System_Init
 *
 * @attention none
 */
...
/**
 * @brief int IMP_Encoder_SetGDRCfg(int encChn, const IMPEncoderGDRCfg *pstGDRCfg);
 *
 * Set encoder channel GDR attribute
 *
 * @param[in] encChn       encode channel num, range:[0, @ref NR_MAX_ENC_CHN - 1]
 * @param[in] pstGDRCfg    GDR attribute pointer
 *
 * @retval 0 success
 * @retval not 0 failure
 *
...
/**
 * @brief int IMP_Encoder_GetGDRCfg(int encChn, IMPEncoderGDRCfg *pstGDRCfg);
 *
 * Get encoder channel GDR attribute
 *
 * @param[in] encChn       encode channel num, range:[0, @ref NR_MAX_ENC_CHN - 1]
 * @param[out] pstGDRCfg   GDR attribute pointer
 *
 * @retval 0 success
 * @retval not 0 failure
 *
...
/**
 * @brief int IMP_Encoder_RequestGDR(int encChn, int gdrFrames);
 *
 * Request GDR frame
 *
 * @param[in] encChn       encode channel num, range:[0, @ref NR_MAX_ENC_CHN - 1]
 * @param[in] gdrFrames    GDR frame num
 *
 * @retval 0 success
 * @retval not 0 failure
 *
 ...
 *
 * Input Yuv Encoder init
 *
 * @param[in] h        Encoder interface
 * @param[in] inWidth  Width
 * @param[in] inHeight Height
 * @param[in] encIn    Encoder attr
 *
 * @retval  0 success
 * @retval  not 0 failure
 *
 ...
 *
 * Input Yuv Encoder process
 *
 * @param[in] h         Encoder interface
 * @param[in] frame     frame
 * @param[out] encOut   stream
 *
 * @retval  0 success
 * @retval  not 0 failure
 *
...
/**
 * @fn int IMP_Encoder_YuvGetCrop(void *h, IMPEncoderCropCfg *pstCropCfg);
 *
 * Input Yuv Encoder Get CropAttr
 *
 * @param[in] h             Encoder interface
 * @param[out] pstCropCfg   Encoder Crop configures
 *
 * @retval  0 success
 * @retval  not 0 failure
 *
 ...
 * @attention none
 */
int IMP_Encoder_YuvGetCrop(void *h, IMPEncoderCropCfg *pstCropCfg);

/**
 * @fn int IMP_Encoder_YuvSetCrop(void *h, IMPEncoderCropCfg *pstCropCfg);
 *
 * Input Yuv Encoder Set CropAttr
 *
 * @param[in] h             Encoder interface
 * @param[in] pstCropCfg    Encoder Crop configures
 *
 * @retval  0 success
 * @retval  not 0 failure
 *
 * @remarks After calling this API, it takes effect at the next I frame
 * @remarks This API modifies the SPS(Sequence Parameter Set) frame_cropping parameters to instruct the decoder to crop unwanted portions
 *
 * @attention none
 */
int IMP_Encoder_YuvSetCrop(void *h, IMPEncoderCropCfg *pstCropCfg);

/**
 * @fn int IMP_Encoder_YuvRequestIDR(void *h);
 *
 * Input Yuv Encoder Request IDR
 *
 * @param[in] h         Encoder interface
 *
 * @retval  0 success
...
/**
 * @fn int IMP_Encoder_SetChnInitQP(int encChn, uint32_t initQP)
 *
 * Set initQP attribute
 *
 * @param[in] encChn        encode channel num, range:[0, @ref NR_MAX_ENC_CHN - 1]
 * @param[in] initQP    	Encoder initQP attribute
 *
 * @retval  0 success
 * @retval  not 0 failure
 *
 * @remarks This API needs to be called after the code channel is created and before calling IMP_Encoder_StartRecvPic for the first time
 *
 * @attention none
 */
