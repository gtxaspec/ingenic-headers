 ...
 *
 * 设置ISP图像上下反转效果功能是否使能,直通编码不支持该接口
 *
 ...
 *
 * 获取ISP图像上下反转效果功能的操作状态,直通编码不支持该接口
 *
 * @param[in] pmode 操作参数指针
 *
 * @retval 0 成功
 ...
 *
 * 设置AWB统计区域的权重，该接口已被弃用，请调用IMP_ISP_Tuning_SetAwbZoneWeight。
 *
 ...
 *
 * 获取AWB统计区域的权重，该接口已被弃用，请调用IMP_ISP_Tuning_GetAwbZoneWeight。
 *
...
*/
typedef struct {
	unsigned int weight[15][15];    /**< 各区域权重信息 [0 ~ 8]*/
} IMPISPZoneWeight;
 ...
 * 设置AWB统计区域的权重。
 *
 * @param[in] awb_zone_weight 各区域权重信息。
 *
 * @retval 0 成功
 * @retval 非0 失败，返回错误码
 *
 ...
 * 获取AWB统计区域的权重。
 *
 * @param[out] awb_zone_weight 各区域权重信息。
 *
 * @retval 0 成功
 * @retval 非0 失败，返回错误码
 *
...
*/
typedef struct {
	unsigned char ae_histhresh[4];    /**< AE统计直方图bin边界 [0 ~ 255],可读/写*/
	unsigned short ae_hist[5];    /**< AE统计直方图bin值 [0 ~ 65535],只读*/
	unsigned char ae_stat_nodeh;    /**< 水平方向有效统计区域个数 [0 ~ 15],只读*/
	unsigned char ae_stat_nodev;    /**< 垂直方向有效统计区域个数 [0 ~ 15],只读*/
} IMPISPAEHist;
 ...
 *
 * @attention 在使用这个函数之前，必须保证IMP_ISP_EnableSensor被执行且返回成功.
 * @attention 该API实现方式为先放大再裁剪.
 * @attention 建议使用IMP_ISP_MultiCamera_Tuning_SetFrontCrop系列接口来实现AutoZoom
 */
...
#define MAXSUPCHNMUN 3  /*ISPOSD最大支持一个通道*/
 ...
 *
 * @attention 这个函数必须在添加sensor之后被调用。
 * @attention 一次只能设置单个ISP的bin文件路径属性。
 */
 ...
 * @retval 非0 失败，返回错误码
 *
 * @attention 在使用这个函数之前，必须保证IMP_ISP_MultiCamera_EnableSensor 和 IMP_ISP_EnableTuning已被调用。
 */
 ...
 *
 * @param[in] num   对应sensor的标号
 * @param[in] isp_core_rgb_coefft_wb_attr   sensor AWB RGB通道偏移参数
 *
 * @retval 0 成功
 * @retval 非0 失败，返回错误码
 *
 ...
 *
 * @param[in] num   对应sensor的标号
 * @param[out] isp_core_rgb_coefft_wb_attr   sensor AWB RGB通道偏移参数
 *
 * @retval 0 成功
 * @retval 非0 失败，返回错误码
 *
...
* @retval 非0 失败，返回错误码
*
* @attention 在使用这个函数之前，IMP_ISP_MultiCamera_Tuning_EnableMovestate已被调用。
*/
 ...
 *
 * 设置AWB统计区域的权重，该接口已被弃用，请调用IMP_ISP_MultiCamera_Tuning_SetAwbZoneWeight。
 *
 * @param[in] num   对应sensor的标号
 * @param[in] awb_weight 各区域权重信息。
 *
 ...
 *
 * 获取AWB统计区域的权重，该接口已被弃用，请调用IMP_ISP_MultiCamera_Tuning_GetAwbZoneWeight。
 *
 * @param[in] num   对应sensor的标号
 * @param[out] awb_weight 各区域权重信息。
 *
 ...
 * @retval 非0 失败，返回错误码
 *
 * @remark 每接收(IMPVI_NUM num, lsize+1)帧就会丢(IMPVI_NUM num, fmark无效位数)帧。
 * @remark 例如：lsize=3,fmark=0x5(IMPVI_NUM num, 每4帧丢第2和第4帧)
 *
 * @attention 在使用这个函数之前，IMP_ISP_MultiCamera_Open已被调用。
 */
 ...
 * @retval 非0 失败，返回错误码
 *
 * @attention 在使用这个函数之前，必须保证IMP_ISP_MultiCamera_EnableSensor被执行且返回成功.
 * @attention 该API实现方式为先放大再裁剪.
 * @attention 建议使用IMP_ISP_MultiCamera_Tuning_SetFrontCrop系列接口来实现AutoZoom
 */
 ...
 *
 * @param[in] num   对应sensor的标号
 * @param[out] IMPISPAutoZoom  自动聚焦配置参数.
 *
 * @retval 0 成功
 * @retval 非0 失败，返回错误码
 *
...
typedef struct {
        unsigned short switch_gpio; /* 双摄 MIPI SWITH控制GPIO */
        uint16_t Msensor_gstate; /* 主摄MIPI SWITH GPIO状态 */
        uint16_t Ssensor_gstate; /* 次摄MIPI SWITH GPIO状态 */
} IMPDoubleSensor;

typedef struct {
        unsigned short switch_gpio[2]; /* 三摄 MIPI SWITCH两个控制GPIO */
        uint16_t Msensor_gstate[2]; /* 主摄MIPI SWITCH 两个控制GPIO状态 */
        uint16_t Ssensor_gstate[2]; /* 次摄MIPI SWITCH 两个控制GPIO状态 */
        uint16_t Tsensor_gstate[2]; /* 三摄MIPI SWITCH 两个控制GPIO状态 */
} IMPTrebleSensor;

typedef struct {
        uint16_t enable;  /* 使能用户自定义MIPI SWITCH控制GPIO */
        uint16_t sensornum; /* sensor数量 */
        union {
                IMPDoubleSensor d; //双摄结构体
                IMPTrebleSensor t; //三摄结构体
        };
} IMPUserSwitchgpio;
 ...
 *
 * 用户设置MIPI SWITH控制GPIO.
 *
 * @param[in] info  GPIO配置属性.
 *
 * @retval 0 成功
 * @retval 非0 失败，返回错误码
 *
 * @attention 在使用这个函数之前，IMP_ISP_Open已被调用，IMP_ISP_AddSensor未调用。
 */
...
typedef enum{
    ...
    //fs0与fs3 拼接
	IMPISP_03_ON_THE_LEFT,               /**< 双摄使能拼接模式，主摄图像在左边 */
	IMPISP_03_ON_THE_RIGHT,              /**< 双摄使能拼接模式，主摄图像在右边 */
	IMPISP_03_ON_THE_ABOVE,              /**< 双摄使能拼接模式，主摄图像在上边 */
	IMPISP_03_ON_THE_UNDER,              /**< 双摄使能拼接模式，主摄图像在下边 */

    //fs1与fs4 拼接
	IMPISP_14_ON_THE_LEFT,              /**< 双摄使能拼接模式，主摄图像在左边 */
	IMPISP_14_ON_THE_RIGHT,             /**< 双摄使能拼接模式，主摄图像在右边 */
	IMPISP_14_ON_THE_ABOVE,             /**< 双摄使能拼接模式，主摄图像在上边 */
	IMPISP_14_ON_THE_UNDER,             /**< 双摄使能拼接模式，主摄图像在下边 */

    //f0与f1与fs3与fs4 一起拼接
	IMPISP_03_14_ON_THE_LEFT =  0x15,    /**< 双摄使能拼接模式，主摄图像在左边 */
	IMPISP_03_14_ON_THE_RIGHT = 0x26,   /**< 双摄使能拼接模式，主摄图像在右边 */
	IMPISP_03_14_ON_THE_ABOVE = 0x37,   /**< 双摄使能拼接模式，主摄图像在上边 */
	IMPISP_03_14_ON_THE_UNDER = 0x48,   /**< 双摄使能拼接模式，主摄图像在下边 */
...
typedef struct  {
	...
	uint16_t outstride[2];        /*[0]:主码流f0 f3 拼接跨度 [1]:次码流f1 f4拼接跨度 */
 ...
 *
 * @param[in] num   对应sensor的标号
 * @param[in] sync_flag 是否使能对齐功能(1为使能，0为失能).
 *
 * @retval 0 成功
 * @retval 非0 失败，返回错误码
 *
 * @attention 在使用这个函数之前，IMP_ISP_EnableTuning已被调用。
 * @attention 要求摄像头的视场角有重叠部分且要尽可能相近，不同色温下不可使用。
 */
