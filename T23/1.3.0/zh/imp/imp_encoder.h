 ...
 *
 * direct_mode=3为三摄三直通模式。此模式下：
 * FrameSource0：主摄主通道；FrameSource3：次摄主通道；Framesource6：三摄主通道
 * 编码Group0绑定到FrameSource0，编码Group1绑定到FrameSource3，编码Group2绑定到Framesource6。
 * 编码Channel0(h264)注册到编码Group0，用于生成主摄主码流。
 * 编码Channel1(h264)注册到编码Group1，用于生成次摄主码流。
 * 编码Channel4(h264)注册到编码Group2，用于生成三摄主码流。
 *
 ...
 *
 * 三摄主通道:                     |--enc_chn 4 用于三摄主通道H264
 * Framesource6 -- Encoder_Group2->
 *                                 |--enc_chn 5 用于三摄主通道JPEG,(不抓取次摄主通道JPEG，请忽略)
 *
 * FrameSource1,FrameSource2,FrameSource4,FrameSource5,Framesource7,Framesource8为非直通通道，没有编码通道和Group的特殊限制。
 */
...
typedef struct {
	...
	int     gdrCycle;       /**< GDR间隔:[3,65535], defualt = 120*/
	int     gdrFrames;      /**< 连续GDR帧的个数:[2,10], default = 6*/
} IMPEncoderGDRCfg;
 ...
 * @remark 如果调用IMP_ISP_SetFrameDrop()函数动态改变系统帧率，那么需要调用该函数修改编码器帧率，完成正确参数配置。
 *
 * @attention 此API只适用于H264编码channel
 */
 ...
 *
 * @param[in] encChn 编码Channel号, 取值范围: [0, @ref NR_MAX_ENC_CHN - 1]
 * @param[in] framelossthd 最大单帧大小(byte)，超过该大小的帧会被丢弃，framelossthd值设置过小可能会导致码流丢帧.
 *
 * @retval 0 成功
 * @retval 非0 失败
 *
 * @remarks 此API需在通道创建之后调用
 *
 * @attention 此API只适用于H264编码channel
 */
 ...
 *
 * @param[in] encChn 编码Channel号, 取值范围: [0, @ref NR_MAX_ENC_CHN - 1]
 * @param[out] framelossthd 最大单帧大小(byte)，超过该大小的帧会被丢弃，framelossthd值设置过小可能会导致码流丢帧.
 *
 * @retval 0 成功
 * @retval 非0 失败
 *
 ...
 *
 * 动态配置I帧和P帧的MaxPictureSize，T23暂不支持单独控制P帧size，MaxPictureSize_P入参内部会被强制对齐为MaxPictureSize_I
 *
 * @param[in] encChn 编码Channel号,取值范围: [0, @ref NR_MAX_ENC_CHN - 1]
 * @param[in] MaxPictureSize_I 最大I帧Size，单位："Kbit"
 * @param[in] MaxPictureSize_P 最大P帧Size，单位："Kbit
 *
 * @retval 0 成功
 * @retval 非0 失败
 *
 ...
 *
 * 动态获取I帧和P帧的MaxPictureSize，T23暂不支持单独控制P帧size，MaxPictureSize_P入参内部会被强制对齐为MaxPictureSize_I
 *
 * @param[in] encChn 编码Channel号,取值范围: [0, @ref NR_MAX_ENC_CHN - 1]
 * @param[out] MaxPictureSize_I 最大I帧Size，单位："Kbit"
 * @param[out] MaxPictureSize_P 最大P帧Size，单位："Kbit
 *
 * @retval 0 成功
 * @retval 非0 失败
 *
 ...
 * 开启分块编码功能。
 *
 * @param[in] mode 分块编码模式：
 * 0、关闭分块编码功能。
 * 1、开启分块编码功能，按照整帧输出。
 * 2、开启分块编码功能，按照码流长度size*cnt输出。
 * mode2模式下可以通过stream->pack[0].frameEnd == 1判断是否是最后一块。
 *
 * @param[in] size 设置编码每块码流buff大小，单位KByte。
 * @param[in] cnt  设置编码码流buff个数,目前只支持1个。
 *
 * @retval 0 成功
 * @retval 非0 失败
 *
 * @remarks 此API需要在系统初始化IMP_System_Init前调用。
 *
 * @attention 无
 */
 ...
 *
 * @param[in] h             编码yuv初始化句柄
 * @param[in] pstCropCfg    编码器裁剪属性
 *
 * @retval  >=0 && < 32     成功
 * @retval  <0              失败
 *
 * @remarks 此API调用后，下一个I帧生效
 * @remarks 此API修改SPS序列参数集的frame_cropping参数，指示解码器裁剪不需要的部分
 *
 * @attention 无
 */
 ...
 *
 * @param[in] encChn        编码Channel号, 取值范围: [0, @ref NR_MAX_ENC_CHN - 1]
 * @param[in] initQP    	编码器initQP属性
 *
 * @retval  =0     			成功
 * @retval  <0              失败
 *
 * @remarks 此API需在编码通道创建后，首次调用IMP_Encoder_StartRecvPic前使用
 *
 * @attention 无
 */
