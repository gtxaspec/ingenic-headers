...
/**
 * @brief IMP_Encoder_YuvInit_Ex(int *h, int inWidth, int inHeight, IMPEncoderYuvIn *encIn);
 *
...
/**
 * @brief IMP_Encoder_YuvEncode_Ex(int h, IMPFrameInfo frame, IMPEncoderYuvOut *encOut);
 *
...
/**
 * @brief IMP_Encoder_YuvExit_Ex(int h);
 *
...
/**
 * @brief IMP_Encoder_VbmAlloc_Ex(uint32_t size, uint32_t align);
 *
...
/**
 * @brief IMP_Encoder_VbmFree_Ex(void *vaddr);
 *
 ...
 * @brief int IMP_Encoder_InputJpege(uint8_t *src, uint8_t *dst, int src_w, int src_h, int q,int *stream_length);
 *
 * Secondary process IVPU external input NV12 encoded JPEG
 *
 * @param[in] *src Source data address pointer
 * @param[in] *dst Bitstream data address pointer
 * @param[in] src_w Image width
 ...
 * @retval not 0 failure
 *
 * @remarks This API only works with NV12 input encoding JPEGs that are 32 wide aligned and 16 high aligned
 *
 */
