...
/**
 * Audio internal and external codec selection
 */
...
/**
 * Define DRC/EQ structure.
 **/
typedef struct {
   int threshold_low;
   int threshold_high;
   float ratio;   int eq_type;
   int freq_low;
   int freq_high;
   int size;
   float* EqTable;
 ...
 * Get audio frame and Raw audio frame
 * @param[in] audioDevId audio device number.
 * @param[in] aiChn audio input channel number.
 * @param[out] frm Audio frame structure pointer
 * @param[out] raw_frm  Raw Audio frame structure pointer
 * @param[in] block Blocking / non blocking identifier.
 ...
 * @attentio Before obtaining Raw data, please call the IMP-AI-EnableGetRaw enable first.
 */
 ...
 *
 * Enable audio input for low pass filtering.
 *
 * @param[in] attr need low pass filtering of audio attribute.
 *
 * @retval 0 success.
 * @retval non-0 failure.
 *
 ...
 *
 * Set the cut-off frequency of low pass filtering for audio input.
 *
 * @param[in] cofrequency the cut-off frequency of low pass filter.
 *
 * @retval 0 success.
 * @retval non-0 failure.
 *
 * @remarks no.
 *
 * @attention while enabling the low-pass filter for audio input,setting the cut-off frequency of the low-pass filter first.
 */
 ...
 *
 * Disable AI low pass filtering function.
 *
 * @param no.
 *
 * @retval 0 success.
...
/**
 * @fn int MP_AI_EnableDrcAndEq(IMPAudioIOAttr *attr,IMPAudioDrcEqConfig* drcEqConfig,int drc_enable, int eq_enable) *
 * enable audio Intput dynamic_range_compression and equalizer feature.
 *
 * @param[in] attr need automatic gain of the audio attribute.
 * @param[in] dynamic range compression threshold ratio. * @param[in] dynamic range compression enable switch.
 * @param[in] equalizer enable switch.
 *
 * @retval 0 success.
 * @retval non-0 failure.
 *
 ...
 *
 * Disable AI dynamic_range_compression and equalizer feature.
 *
 * @param no.
 *  * @retval 0 success.
 * @retval non-0 failure.
 ...
 * @param[in] audioDevId audio device number.
 * @param[in] aiChn audio input channel number.
 * @param[in] codec audio channel parameters. SELECT_EXTERNAL：external codec  SELECT_INTERNAL: internal codec
 ...
 * @attentio
	1. This function should be called after IMP_AI_Enable, IMP_AI_EnableChn, IMP_AI_SetGain.
	2. This function does not support dynamic switching during recording, playback, and streaming.
	3. If this function is not called, the internal codec is used by default.
 */
