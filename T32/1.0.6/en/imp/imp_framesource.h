 ...
 * *Channel 0 : Ultra clear video stream
 * *Channel 1 : High clear video stream, or IVS can only do the algorithm of data source
 * *Channel 2 : Standard clear video stream, or Jpeg snap data source
 *
...
typedef enum {
	...
	FS_INJ_CHANNEL,         /**< injection frame channel */
} IMPFSChnType;
 ...
 *
 * Set the size of ivdc_mem_line for single sensor ivdc mode, this parameter affects memory usage
 *
 * @param[in] ivdc_mem_line The larger the parameter value, the more memory is used, and the more stable the encoding becomes.
 *
 * @retval 0 means success.
 * @retval Other values means failure, its value is an error code.
 *
 ...
 *
 * @attention
 * 1.This function must be called before IMP_FrameSource_EnableChn.
 * 2.Only supports single sensor ivdc mode.
 */
 ...
 *
 * Set whether to copy mode to obtain images
 *
 * @param[in] chnNum  Id of frame channel
 * @param[in] b_nocopy_depth  Copy mode or not
 *
 * @retval 0 means success.
 * @retval Other values means failure, its value is an error code.
 *
 ...
 *
 * Create ExternInject Chn
 *
 * @param[in] chnNum Channel ID
 * @param[in] chnAttr Channel info point
 *
 * @retval 0 success
 * @retval non-0 failure，its value is an error code
 *
 * @remark Create simulation channels, import external NV12 data, and provide data sources for backend modules; \n
 * You can set the relevant properties of the channel, including image width, image height, image format, output frame rate of the channel, cache buf number, and not support cropping and scaling properties。\n
 *
 * @attention none.
 */
 ...
 *
 * Destroy external imported video source channel
 *
 * @param[in] chnNum Channel ID
 *
 * @retval 0 success
 * @retval non-0 failure，its value is an error code
 *
 * @remark Destruction channel
 *
 * @attention If the program has called IMP_FrameSource_ExternInject_EnableChn，Be sure to callIMP_FrameSource_ExternInject_DisableChn after, then use this function again.
 */
 ...
 * @remark none.
 *
 * @attention Before using this function, it is necessary to ensure that the enabled channel has been created.
 */
 ...
 *
 * Obtain channel idle buffer frame
 *
 * @param[in] chnNum Channel ID.
 *
 * @param[out] frame Frame info of channel idle buffer
 *
 * @retval 0 success
 * @retval non-0 failure，its value is an error code
 *
 ...
 *
 * Add frame information of external video data to FrameSource
 *
 * @param[in] chnNum Channel ID.
 *
 * @param[in] frame External video frame information.
 *
 * @retval 0 success
 * @retval non-0 failure，its value is an error code
 *
 * @remark IMP-FrameSource-DequueBuffer takes an idle buffer from FrameSource, modifies the actual frame data externally, and places it in Framesource through IMP-FrameSource_QueueBuffer to complete the next IVS/OSD/Encoder
 *
 * @attention DequeueBuffer and QueueBuffer appear in pairs
 */
 ...
 *
 * Suspend the created Framesource channel, and cache it while waiting for completion.
 *
 * @param[in] chnNum
 *
 * @retval 0 success
 * @retval non-0 failure，its value is an error code
 *
 * @attention Call this before suspend
 */
 ...
 *
 * Resume the suspended Framesource channel.
 *
 * @param[in] chnNum
 *
 * @retval 0 success
 * @retval non-0 failure，its value is an error code
 *
 * @attention  Call this after waking up.
 */
 ...
 *
 * Get the direct_made parameter of the driver
 *
 ...
 * @retval non-0 failure，its value is an error code
 *
 * @attention After IMP_System_Init initialization
 */
