...
typedef enum {
	IMP_ROI_QPMODE_DELTA         = 0,    /**< ROI Relative QP Mode */
	IMP_ROI_QPMODE_FIXED_QP      = 1,    /**< ROI Absolute QP Mode , Unsupported */
...
typedef struct {
	...
	int8_t                       qp;    /**< mode=IMP_ROI_QPMODE_DELTA,Relative_QP [-26~25]. Unsupported Absolute_QP */
} IMPEncoderRoiWin;
...
typedef struct {
	...
	int                          mapSize;
	IMPEncoderQPMode             mode;
	int8_t                       Quality;    /**< mode=IMP_ROI_QPMODE_DELTA,Relative_QP [-26~25]. Unsupported Absolute_QP */
	int32_t                      reserved;
 ...
 *
 * obtain ROI attr that was successfully set last time.
 *
 * @param[in] encChn        Channel ID.
 * @param[out] RoiAttr      ROI Attr.
 *
 * @retval  >=0 && < 32     success.
 * @retval  <0              failed.
 *
...
/**
 * @brief IMP_Encoder_SetChnMapRoi(int encChn, IMPEncoderMapRoiAttr *pstMapRoi)
 * Set Map ROI.
 *
 * @param[in] encChn        Channel ID.
 * @param[in] pstMapRoi     MapRoi Attributes
 *
 * @retval  >=0 && < 32     success.
 * @retval  <0              failed.
 *
 ...
 *
 * @attention:
 * pstMapRoi->map      Map array address.
 * pstMapRoi->mapSize  Map array length.
 * pstMapRoi->mode     ROI_QP mode.
 * pstMapRoi->Quality  Qp value.if mode=IMP_ROI_QPMODE_DELTA，ROI_QP is Relative_QP[-26~25]. Unsupported Absolute_QP.

 * Map is the address of the array, which divides a frame of an image into an array of mapSize length.
 * Each array element has a value of 0: do not set ROI_QP; Value 1: ROI_QP set to Quality
 * MapSize is related to H264/h265 and encoding resolution:
 * In ROI mode: H265 has a macro block size of 32 * 32, and H264 has a macro block size of 16 * 16.
 * H265/1920 * 1080 as an example,mapSize = 1920/32 * 1080/32 = 60*34 = 2040.
 *
 * In map[i], the top 6 bits represent the QP value, and the bottom 2 bits indicate the mode.
 * bottom 2 bits:
 * 00:Not using Map_Roi mode, high 6bit invalid.
 * 01:Using Map_Roi mode, all macro block QP values are input parameter pstMapRoi->Quality, and high 6bits are invalid.
 * 10:Using Map-Roi relative mode, the current macro block QP is determined by the high 6bit of the map, using complement representation.
 *
 * high 6 bits:
 * The range of 6bits when using Map-Roi relative mode is [-32,31], and the complement representation is used to represent positive and negative numbers:111111=-1;111110=-2;111101=-3;...;100000=-32;
 * Values are 32, 33, 34,..., 63; Corresponding relative QP: -32, -31, -30,...,-1;
 * Values are 0, 1, 2,...,31; Corresponding relative QP: 0, 1, 2,...,31;
 *
 * for example:
 * Three macro blocks use Map-Roi relative mode, with relative Qp values of -1, -2, 15.
 * map[i] = (63 << 2) | 0x2; map[i+1] = (62 << 2) | 0x2; map[i+2] = (15 << 2) | 0x2;
 *
 * Reference sample-Encoder-video-Roi.c
 */
 ...
 *
 * Alloc a contiguous physical address of memory.
 *
 ...
 *
 * Free a contiguous physical address of memory.
 *
 * @param[in] vaddr         Physical address’s Virtual address
 *
 * @retval  >=0 && < 32     success.
 * @retval  <0              failed.
 *
 ...
 *
 * Finding physical addresses through virtual addresses.
 *
 * @param[in] vaddr         Physical address’s Virtual address
 *
 * @retval  >=0 && < 32     success.
 ...
 *
 * Finding virtual addresses through physical addresses.
 *
 ...
 *
 * Unbound encoding initialization.
 *
 * @param[in] h             Encoding handle
 * @param[in] inWidth       Encoding Width
 * @param[in] inHeight      Encoding Height
 * @param[in] encIn         Encoding Attribute
 *
 * @retval  >=0 && < 32     success.
 * @retval  <0              failed.
 *
 ...
 * @param[out] encOut       Bitstream Information
 *
 * @retval  >=0 && < 32     success.
 * @retval  <0              failed.
 *
